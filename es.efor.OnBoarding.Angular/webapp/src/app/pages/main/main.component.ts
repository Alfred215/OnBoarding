 ...
 // Ejemplo de como montar la navegación para coreUI
  ...
  async fillNav() {
    ...
    for (const navItem of navByRoles) {
      ...
      if (navItem.roles.length === 0 || navItem.roles.some(r => userRoles.indexOf(r.toString()) >= 0)) {
        ...
        if (navItem.childsWithRole != null) {
          ...
          for (const navChildItem of navItem.childsWithRole) {
            ...
            if (navChildItem.roles.length === 0 || navChildItem.roles.some(r => userRoles.indexOf(r.toString()) >= 0)) {
